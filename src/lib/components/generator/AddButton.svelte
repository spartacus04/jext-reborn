...
<div class="flex p-3 mb-2">
	<button
		class="flex-1 text-[#aeaeae] bg-[#404040] hover:bg-[#505050] p-2 flex justify-center border-black border-2"
		use:inputFile={{
			accept: '.zip,.nbs,.mp3,.wav,.ogg,.aac,.flac,.json',
			multiple: true,
			cb: createNewDisc
		}}
		use:dropFile={{
	...
	{#if isTauri}
		<button
			class="ml-4 aspect-square text-[#aeaeae] bg-[#404040] hover:bg-[#505050] p-2 flex justify-center border-black border-2"
			on:click={() => downloadModal.openModal()}
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-14 w-14"
			...
			>
				...
				<path stroke-width="2" d="M5 21h14" />
			</svg>
		</button>
	{/if}
</div>
