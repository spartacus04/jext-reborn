<script lang="ts">
	...
	const getRecentExportsPromise = async () => {
		...
		const sortedExports = recentExports.sort(
			(a, b) => getDateTime(b).getTime() - getDateTime(a).getTime()
		);

		return await Promise.all(
			sortedExports.map(async (filename) => {
...
{:then recentExports}
	...
	{:else}
		<div
			class="grid p-4 pt-2 gap-2 grid-cols-1 xs:grid-cols-2 lg:grid-cols-1 overflow-y-auto overflow-x-hidden"
		>
			...
			{#each recentExports as exportRp}
				...
				>
					<div class="flex flex-col lg:flex-row items-center w-full flex-shrink lg:max-w-[85%]">
						<img
							src={`data:image/png;base64, ${exportRp.icon}`}
							alt={exportRp.filename}
							class="w-28 lg:w-20 lg:h-20 rounded-lg"
						/>
						<div class="lg:ml-4 block flex-shrink overflow-x-auto w-full">
							...
							>
								{exportRp.name} - {exportRp.date.toLocaleDateString()}
								{exportRp.date.toLocaleTimeString()}
							</h3>
							<p
