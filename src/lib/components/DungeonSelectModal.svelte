		...
		<svelte:fragment slot="panel">
			{#if tabSet === 0}
				<div class="grid sm:grid-cols-4 p-2 gap-2">
					{#each Object.entries(items) as [key, val]}
						{#if isBoolean}
							<button class="card rounded-lg flex flex-col justify-between [&>*]:pointer-events-none outline-none {value && +getGroupValue(key) > 0 ? "variant-filled-tertiary" : ""}" data-type="group" data-target={key} on:click={updateBoolean}>
								...
								<footer class="card-footer">
									{#if isChance}
										<div class="input appearance-none flex items-center">
											<input type="number" class="input text-left appearance-none outline-none" min="-1" max="1000" data-type="group" data-target={key} on:input={update} placeholder="-" value={+getGroupValue(key)}>
											{#key flag}
									...
									{:else}
										<input type="number" class="input text-center appearance-none" min="-1" max="12" data-type="group" data-target={key} on:input={update} placeholder="-" value={getGroupValue(key)}>
									{/if}
								</footer>
							</div>
						{/if}
			...
			{:else if tabSet === 1}
				<div class=" h-full grid grid-cols-1 sm:grid-cols-2">
					...
					<h3 class="h3 hidden sm:flex w-full">{isChance ? 'Disc chance to be found' : 'Max. number of discs'}</h3>
					...
					{#each Object.entries(value) as [key, val]}
						...
						{#if isBoolean}
							<div class="flex items-center">
								<input type="checkbox" class="checkbox aspect-square" data-type="item" data-target={key} on:change={updateBoolean} checked={val === 1}>
								<span class="text-sm mx-2">Enabled</span>
							</div>
						{:else}
							{#if isChance}
								<div class="input appearance-none flex items-center">
									<input type="number" class="input" min="0" max="1000" value={val} data-type="item" data-target={key} on:input={update} placeholder="0">
									{#key flag}
										<span class="text-sm mx-2">
											{(val / 10)}%
										</span>
