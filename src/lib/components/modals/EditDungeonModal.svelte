<script lang="ts">
    ...
    const update = (e: Event) => {
		...
		if (type === 'group') {
            if(lootTables.filter((item) => group === `${item.displayName}-${item.subtitle}`)[0].mode == 'percentage' && +target.value > 1000) {
                target.value = '1000';
            }

			const elements = lootTables.filter((item) => group === `${item.displayName}-${item.subtitle}`)[0].contents;

			elements.forEach((item) => {
				dungeons[item] = parseInt(target.value);
			});
		} else {
            if(lootTables.filter((item) => item.contents.includes(group))[0].mode == 'percentage' && +target.value > 1000) {
                target.value = '1000';
            }
        }
	...
	const getGroupValue = (group: Loottable): string => {
		const elements = lootTables.filter((item) =>  `${item.displayName}-${item.subtitle}` === `${group.displayName}-${group.subtitle}`)[0].contents;
    ...
    const getPercentage = (value: number, group: Loottable) => {
		...
		if(group.mode == 'percentage') {
			...
			result = Math.round((value / ((group.defaultLoottableWeight! + value) * (1 - (group.ignorePercentage ?? 0)))) * 1000) / 10;
		}
...
>
	...
	>
        ...
        <div>
                    ...
                    {#each lootTables as lootTable}
                        <div class="flex flex-col p-4 bg-surface-background-variant">
                            ...
                            <div class="flex items-end flex-1">
                                <div class="flex items-baseline w-full px-2 pt-0.5 pb-1 text-white bg-[#0e0e0e] border-2 border-[#0dd166] rounded-sm focus-within:border-white focus-within:outline-none text-lg">
                                    <input
                                        type="number"
                                    ...
                                    />
                                    {#key flag}
                                        <span class="text-sm border-l-2 border-l-surface-separator pl-2 w-20 text-right">
            ...
            {:else}
                {#each Object.entries(dungeons) as [key, _]}
                    <div class="flex items-baseline w-full px-2 py-1 text-white bg-[#0e0e0e] border-2 border-[#0dd166] rounded-sm focus-within:border-white focus-within:outline-none text-lg">
                        <!-- group icon -->
                        <img src={keyToGroup(key).img} alt="dungeon" class="w-8 h-8 aspect-square self-center" />
                        <!-- group icon -->
                        <img src={groups[keyToGroup(key).group]} alt="group" class="self-center aspect-square w-8 h-8 mr-2" />
		...
		<div class="flex gap-2 pt-4 justify-end text-white font-minecraft font-bold">
            <LauncherButton text={advancedMenuOpen ? 'Basic' : 'Advanced'} type="tertiary" on:click={() => advancedMenuOpen = !advancedMenuOpen} />
