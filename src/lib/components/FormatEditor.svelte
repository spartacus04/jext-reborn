<script lang="ts">
	...
	$: (output = MCText.toHTML(
		text
			...
			.replaceAll('</div>', '')
			.replaceAll('<br>', '')
	).replaceAll('amp;', '&')) && setTimeout(() => MCText.refeashObfuscate(previewDiv), 1);

	$: split = text.replaceAll('</div>', '').replaceAll('<br>', '').split('<div>');
...
<div class="flex h-min flex-col border-mc-light-gray border rounded-md">
		...
		<button class="[&>*]:pointer-events-none" on:click={() => insertSymbol('k')}>
			<svg
				...
				/><path d="M14 21h1" /><path d="M3 9v1" /><path d="M21 9v1" /><path d="M3 14v1" /><path
					d="M21 14v1"
				/><line x1="7" x2="15" y1="8" y2="8" /><line x1="7" x2="17" y1="12" y2="12" /><line
					x1="7"
					x2="13"
					y1="16"
					y2="16"
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 mb-2">
		<div
			class="h-full bg-transparent resize-none border-0 shadow-none outline-none border-none m-1 p-2 before:empty:content-[attr(placeholder)] before:empty:focus:content-[''] text-mc-light-gray"
		...
		<div
			...
			class="font-minecraft p-2 bg-[#0f0110] -mb-2 rounded-b-md sm:rounded-bl-none"
		>
...
<div data-popup="colorPopup" class="variant-filled-primary p-2 rounded-md">
	<div
		class="grid grid-cols-4 gap-2 [&>*]:w-4 [&>*]:aspect-square [&>*]:cursor-pointer [&>*]:border [&>*]:border-transparent"
	>
