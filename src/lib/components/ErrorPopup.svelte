...
<main class="bg-surface-500 rounded-md p-2 h-[50%] overflow-y-auto w-[90%] sm:w-[80%] lg:w-[50%]">
        ...
        <Accordion>
            <AccordionItem>
                ...
                <svelte:fragment slot="summary"
                    >My requests keep failing!</svelte:fragment
                >
                <svelte:fragment slot="content">
                    <p>
                        Due to the nature of HTTPS, some requests may fail. This is because the server is running on HTTP and the browser is blocking the request. To fix this, you can download the JEXT Desktop App and run it on your computer. This will allow you to connect to the server without any issues.
                    </p>
                    {#await getDesktopAppDownload}
						<p></p>
					{:then url}
						{#if url != null}
							(<a href={url} target="_blank" rel="noopener noreferrer" class="underline">Download the desktop app here</a>)
						{/if}
					{/await}
                </svelte:fragment>
            </AccordionItem>
    ...
    <div class="flex w-full justify-around  ">
        ...
        <a class="btn variant-filled-primary" href="https://support.google.com/admanager/answer/10358597?hl=en" target="_blank" rel="noopener noreferrer">Export .har file</a>
    </div>
</main>
