...
<main class="bg-surface-500 rounded-md p-2 h-[50%] overflow-y-auto w-[90%] sm:w-[80%] lg:w-[50%]">
	<Stepper on:complete={complete}>
		<Step locked={resourcePackStatus == "NotValid"}>
			...
			<FileDropzone name="ResourcePack" bind:files={files} accept=".zip">
		...
		<Step locked={resourcePackStatus != "JextRP" && discsStatus == "NotValid"}>
			...
			{:else}
				...
				{#if discsStatus == "Old"} 
					<p class="variant-filled-error p-2 rounded-lg">Old version of discs.json detected, loot tables for discs and fragments will be reset!</p>
				{/if}
			{/if}
		</Step>
	</Stepper>
