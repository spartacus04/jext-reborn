...
export const renderer: RendererObject = {
	...
	link: (href: string, _: string | null | undefined, text: string) =>
		`<a href="${href}" class="a">${text}</a>`,
	list: (body: string, ordered: boolean, _: number | '') =>
		ordered
			? `<ol class="m-4 ol list-decimal">${body}</ol>`
			: `<ul class="m-4 ul list-disc">${body}</ul>`,
	listitem: (text: string) => `<li class="m-4 li">${text}</li>`,
	image: (href: string, _: string | null | undefined, text: string) =>
		`<img src="${href}" alt="${text}" class="img">`
};
