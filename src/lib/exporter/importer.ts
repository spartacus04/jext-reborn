...
export const JextReader = async (blob: Blob): Promise<BaseDisc[]> => {
    ...
    const musicDiscs = await Promise.all(parsedMusicDiscs.map(async (element) => {
        const namespace = element['disc-namespace'];

        const retrievedFiles = {
            discTexture: await zip.file(`assets/minecraft/textures/item/music_disc_${namespace}.png`)!.async('blob'),
            fragmentTexture: await zip.file(`assets/minecraft/textures/item/fragment_${namespace}.png`)!.async('blob'),
    ...
    const nbsDiscs = await Promise.all(parsedNbsDiscs.map(async (element) => {
        const namespace = element['disc-namespace'];
