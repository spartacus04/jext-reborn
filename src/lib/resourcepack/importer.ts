...
export const importRP = async (discStore: Writable<Disc[]>, rp: Blob, discsJson: Disc[]) => {
	...
	discStore.set(
		await Promise.all(
			discsJson.map(async (disc) => {
				const namespace = disc['disc-namespace'].substring(disc['disc-namespace'].indexOf('.') + 1);

				const track = await zip
					.file(`assets/minecraft/sounds/records/${namespace}.ogg`)!
					.async('blob');
				...
				const texture =
					(await zip
						...
						?.async('blob')) ??
					new Blob([await (await fetch(default_disc)).blob()], { type: 'image/png' });

				const fragmentTexture =
					(await zip
						.file(`assets/minecraft/textures/item/fragment_${namespace}.png`)
						?.async('blob')) ??
					new Blob([await (await fetch(default_fragment)).blob()], { type: 'image/png' });
	...
	resourcePackStore.set({
		...
		description: JSON.parse(await zip.file('pack.mcmeta')!.async('text')).pack
