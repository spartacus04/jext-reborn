...
export const processResources = async (ffmpeg: FFmpeg|null, onError: (err : string) => unknown, onProgress: (total: number|undefined, done: number|undefined, status: string|undefined, index: number) => unknown) => {
    ...
    for (const [i, disc] of discs.entries()) {
        if(disc.uploadData) {
            ...
            const track = await prepareAudio(disc.uploadData.uploadedTrack, {
...
export const generateResourcePack = async () : Promise<Blob> => {
    ...
    discs.forEach(disc => {
		soundsjson[`music_disc.${disc["disc-namespace"]}`] = {
			sounds: [
				{
    ...
    rp.file('assets/minecraft/models/item/music_disc_11.json', JSON.stringify({
    ...
    rp.file('assets/minecraft/models/item/disc_fragment_5.json', JSON.stringify({
        parent: 'item/generated',
        textures: {
    ...
    for(const disc of discs) {
        // disc model
        rp.file(`assets/minecraft/models/item/music_disc_${disc["disc-namespace"]}.json`, JSON.stringify({
            parent: 'item/generated',
			textures: {
        ...
        // fragment model
        rp.file(`assets/minecraft/models/item/fragment_${disc["disc-namespace"]}.json`, JSON.stringify({
            parent: 'item/generated',
            textures: {
        ...
        // disc texture
        rp.file(`assets/minecraft/textures/item/music_disc_${disc["disc-namespace"]}.png`, disc.packData!.texture);

        // fragment texture
        rp.file(`assets/minecraft/textures/item/fragment_${disc["disc-namespace"]}.png`, disc.packData!.fragmentTexture);

        // track
        rp.file(`assets/minecraft/sounds/records/${disc["disc-namespace"]}.ogg`, disc.packData!.track);
    }

    rp.file('jext.json', JSON.stringify(
        discs.map(disc => {
...
export const generateGeyserResourcePack = async () : Promise<Blob> => {
    ...
    rp.file('sounds/sound_definitions.json', JSON.stringify({
    ...
    for(const disc of discs) {
        ...
        rp.file(`textures/items/fragment_${disc["disc-namespace"]}.png`, disc.packData!.fragmentTexture);
