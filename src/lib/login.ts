...
export const login = async (modalStore: ModalStore, data: { ip: string|undefined|null, port: number|undefined|null }): Promise<boolean> => {
    const ip = data.ip ?? await new Promise<string>((resolve) => {
        modalStore.trigger({
            type: 'prompt',
    ...
    const port = data.port ?? await new Promise<number>((resolve) => {
        modalStore.trigger({
            type: 'prompt',
            title: 'Connect to jext server',
    ...
    const response = await fetch(`http://${ip}:${port}/connect`, {
        ...
        console.error(e)
        await new Promise<void>(async (resolve) => { modalStore.trigger({
            type: 'alert',
...
export const logout = async (force: boolean): Promise<void> => {
    ...
    await fetchAuthed('disconnect', {
        ...
        console.error(e)
        await new Promise<void>(async (resolve) => { getModalStore().trigger({
            type: 'alert',
